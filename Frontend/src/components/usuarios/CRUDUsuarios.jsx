...
const CRUDUsuarios = () => {
	...
	const add = () => {
		Axios.post('/usuarios', { nombre, contrasena, usuario, rol }).then(() => {
			getUsuarios();
			limpiarCampos();
			Swal.fire({
	...
	const update = () => {
		Axios.put(`/usuarios/${id}`, { nombre, contrasena, usuario, rol }).then(() => {
			getUsuarios();
			limpiarCampos();
			Swal.fire({
				...
				<tbody>
					{usuarios.map((val, key) => (
						<tr key={key}>
							...
							<td>{val.rol}</td>
							<td>
								<button type='button' onClick={() => editarUsuario(val)} className='btn btn-primary'>
									Editar
								</button>
								<button type='button' className='btn btn-danger' onClick={() => deleteUsuario(val)}>
									Eliminar
								</button>
							</td>
						</tr>
